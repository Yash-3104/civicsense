        ...
        >
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm font-semibold text-white">
              {event.type === "ISSUE_ESCALATED" ? "SLA ESCALATED" : event.type?.replaceAll("_", " ") || "Realtime Event"}
            </p>
          ...
          <p className="mt-2 text-sm text-zinc-300">
            {event.type === "ISSUE_ESCALATED" ? event.title || event.message || "SLA breached/escalated for a task" : event.title || event.message || "Worker queue updated"}
          </p>
        </div>
      ))}
    </div>
